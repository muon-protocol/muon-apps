...
const VALID_CHAINS = [
  '1', '137', '42161', '43114', '250', '56'
]
const SCALE = new BN('1000000000000000000')
const GRAPH_URL_MAPPING = {
...
const GRAPH_DEPLOYMENT_MAPPING = {
...
async function tokenVWAP(token, pairs, chainId, metadata) {
  ...
  for (let i = 0; i < pairs.length; i++) {
    ...
    pairVWAPPromises.push(pairVWAP(pairs[i], index, chainId))
  }
...
async function pairVWAP(pair, index, chainId) {
  return getTokenTxs(
    pair, 
    GRAPH_URL_MAPPING[chainId], 
    GRAPH_DEPLOYMENT_MAPPING[chainId]
  ).then((tokenTxs) => {
...
async function LPTokenPrice(token, pairs0, pairs1, chainId) {
  ...
  let resultDecimals = await multiCall(chainId, contractCallContextDecimal)
  ...
  let _tokenVWAPResults = await Promise.all([
    pairs0.length ? tokenVWAP(metadata.t0, pairs0, chainId, pairs0Metadata) : null,
    pairs1.length ? tokenVWAP(metadata.t1, pairs1, chainId, pairs1Metadata) : null
  ])
...
module.exports = {
  APP_NAME: 'sushi_permissionless_oracles_vwap',
  APP_ID: 22,
  ...
  onRequest: async function (request) {
    ...
    switch (method) {
      case 'price':
        let { token, pairs, chainId, hashTimestamp } = params
        ...
        let { price, volume } = await tokenVWAP(token, pairs, chainId)
        return {
          token: token,
      ...
      case 'lp_price': {
        let { token, pairs0, pairs1, chainId, hashTimestamp } = params
        if(!VALID_CHAINS.includes(chainId))
        {
          throw { message: 'INVALID_CHAIN' }
        }
        ...
        const { price, sumVolume } = await LPTokenPrice(token, pairs0, pairs1, chainId)
        return {
          token: token,
          tokenPrice: price,
          pairs0: pairs0,
